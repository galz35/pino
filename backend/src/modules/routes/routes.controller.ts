...
export class RoutesController {
  ...
  findAll(@Query('storeId') storeId: string, @Query('vendorId') vendorId?: string) {
    return this.service.findAll(storeId, vendorId);
  }

  @Post()
  @ApiOperation({ summary: 'Crear ruta de vendedor' })
  create(@Body() dto: { storeId: string; vendorId: string; clientIds?: string[]; date?: string; notes?: string; status?: string }) {
